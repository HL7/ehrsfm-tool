...
The text contains non-printable characters [<xsl:text /><xsl:value-of select="$non-printing-text2" /><xsl:text />].</svrl:diagnostic-reference></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:apply-templates select="*" mode="M9" /></xsl:template><xsl:template match="text()" priority="-1" mode="M9" /><xsl:template match="@*|node()" priority="-2" mode="M9"><xsl:apply-templates select="*" mode="M9" /></xsl:template>
...
Function must have one or more conformance criteria.</svrl:diagnostic-reference></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:variable name="local-alias" select="substring-after($thing-id, $parent-alias)" />
