...
A Header must have at least one descendant function.</svrl:diagnostic-reference></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:variable name="start-string" select="concat(substring($statement, 1, 25), '...')" />
...
The text is not single spaced (there are multiple spaces).</svrl:diagnostic-reference></svrl:failed-assert></xsl:otherwise></xsl:choose><xsl:variable name="has-reference" select="contains(notes/text(), 'conform to function')" /><xsl:variable name="text-with-ref" select="substring-after(notes/text(), 'conform to function ')" /><xsl:variable name="ref-id" select="substring-before($text-with-ref, ' ')" />
		...
		<!--REPORT -->
<xsl:if test="$has-reference and count(../object[alias/text()=$ref-id]) = 0"><svrl:successful-report test="$has-reference and count(../object[alias/text()=$ref-id]) = 0" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:schold="http://www.ascc.net/xml/schematron"><xsl:attribute name="location"><xsl:apply-templates select="." mode="schematron-get-full-path" /></xsl:attribute><svrl:text><xsl:text /><xsl:value-of select="id" /><xsl:text /> <xsl:text /><xsl:value-of select="$ref-id" /><xsl:text /></svrl:text> <svrl:diagnostic-reference diagnostic="FMFLC16"><xsl:attribute name="xml:lang">en</xsl:attribute>
...
The dependent criteria is missing the required phrase 'according to scope of practice, organizational policy and/or jurisdictional law'.</svrl:diagnostic-reference></svrl:successful-report></xsl:if>
		...
		<!--REPORT -->
<xsl:if test="not($is-dependent) and contains(notes, 'according to scope of practice, organizational policy and/or jurisdictional law')"><svrl:successful-report test="not($is-dependent) and contains(notes, 'according to scope of practice, organizational policy and/or jurisdictional law')" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:schold="http://www.ascc.net/xml/schematron"><xsl:attribute name="location"><xsl:apply-templates select="." mode="schematron-get-full-path" /></xsl:attribute><svrl:text><xsl:text /><xsl:value-of select="$id" /><xsl:text /></svrl:text> <svrl:diagnostic-reference diagnostic="FMDC002"><xsl:attribute name="xml:lang">en</xsl:attribute>
The dependent criteria phrase is present in when the criteria is not designated as dependent.</svrl:diagnostic-reference></svrl:successful-report></xsl:if>

		<!--REPORT -->
<xsl:if test="$is-dependent and tag[@name='Optionality' and not(@value='SHALL')]"><svrl:successful-report test="$is-dependent and tag[@name='Optionality' and not(@value='SHALL')]" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:schold="http://www.ascc.net/xml/schematron"><xsl:attribute name="location"><xsl:apply-templates select="." mode="schematron-get-full-path" /></xsl:attribute><svrl:text><xsl:text /><xsl:value-of select="$id" /><xsl:text /></svrl:text> <svrl:diagnostic-reference diagnostic="FMDC003"><xsl:attribute name="xml:lang">en</xsl:attribute>
Dependent Criteria should have an optionality of 'SHALL'.</svrl:diagnostic-reference></svrl:successful-report></xsl:if><xsl:variable name="option-value" select="tag[@name='Optionality']/@value" /><xsl:variable name="text-without-optionality" select="concat(substring-before(notes/text(), $option-value), substring-after(notes/text(), $option-value))" />
