    ...
    public class MAXImporter3 : Util
    {
        ...
        public bool import(EA.Repository Repository, EA.Package selectedPackage, string fileName)
        {
            ...
            using (stream)
            {
                ...
                window.setup(model.objects.Count() + model.relationships.Count());
...
/*                    if (eaElement.TaggedValues.GetByName("MAX::SourceFile") == null)
                    {
                        EA.TaggedValue tvSource = ((EA.TaggedValue)eaElement.TaggedValues.AddNew("MAX::SourceFile", ""));
                        tvSource.Value = sourceXmlName;
                        tvSource.Update();
                    }
